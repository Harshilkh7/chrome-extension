...
async function createTokenPair(user) {
  ...
  const accessToken = jwt.sign({ id: user._id.toString(), type: 'access', sid: hashJti(jti) }, getAccessSecret(), { expiresIn: ACCESS_TTL });

  const refreshExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
  await RefreshSession.create({ userId: user._id, jtiHash: hashJti(jti), expiresAt: refreshExpiresAt });
...
async function rotateRefreshToken(refreshToken) {
  ...
  const session = await RefreshSession.findOne({ jtiHash: hashJti(decoded.jti), userId: decoded.id, revokedAt: null });
  if (!session || session.expiresAt <= new Date()) throw new Error('Refresh session is revoked or expired');
...
const refreshToken = async (req, res) => {
  try {
    ...
    if (!cookies[REFRESH_COOKIE]) return res.status(401).json({ error: 'Refresh token required' });
...
const logout = async (req, res) => {
  try {
    ...
    res.append('Set-Cookie', clearCookie(ACCESS_COOKIE));
    res.append('Set-Cookie', clearCookie(REFRESH_COOKIE));
